...
function TimelineItem({item, sisterItem, isNew, phaseKind, isFeedLast, sisterPlayAnimation, onSisterCycleComplete, firstDropAnim, onFirstDropLand, onFirstDropComplete}){
  ...
  } else if(item.kind === 'scene1-ask-record'){
    const Scene1AskRecordCard = window.Scene1AskRecordCard;
    body = Scene1AskRecordCard ? <Scene1AskRecordCard entry={item} isNew={isNew}/> : null;
  ...
  return (
    <TimelineRailNode
      ...
      nodeKind={(item.kind === 'record-group' || item.kind === 'scene1-ask-record') ? (item.ai && !item.primary ? 'sister-card' : 'voice-card') : (sisterItem ? (item.kind === 'sync-card' ? 'sync-card' : 'voice-card') : item.kind)}

...
function SmartCaptureTimelineCard({item, isNew}){
  const [open, setOpen] = React.useState(item.defaultOpen !== false);
  const [playing, setPlaying] = React.useState(false);
  const records = item.records || [];

  return (
    <article className={'tl-smart-capture'+(open ? ' is-open' : ' is-collapsed')+(isNew ? ' fade-in' : '')}>
      <div className="tl-smart-source-head">
        <span className="tl-smart-source-label"><i/>用户原声</span>
        <time>{item.time}</time>
      </div>
      <div className="tl-smart-source-row">
        <button
          type="button"
          className={'tl-smart-voice'+(playing ? ' is-playing' : '')}
          onClick={()=>setPlaying(value=>!value)}
          aria-label={playing ? '暂停原声' : '播放原声'}
        >
          <span className="tl-smart-voice-icon" aria-hidden="true">{playing ? 'Ⅱ' : '▶'}</span>
      ...
      <button
        type="button"
        className="tl-smart-toggle"
        onClick={()=>setOpen(value=>!value)}
        aria-expanded={open}
      >
        <span className="tl-smart-spark" aria-hidden="true">✦</span>
        <span className="tl-smart-toggle-copy"><b>AI 已整理</b><small>{records.length} 条记录 · {new Set(records.map(record=>record.subject)).size} 个对象</small></span>
        <span className="tl-smart-chevron" aria-hidden="true">⌄</span>
      </button>

      {open && (
        <div className="tl-smart-result">
          <div className="tl-smart-records">
            {records.map((record, index)=>(
              <div className="tl-smart-record-row" key={`${record.subject}-${record.item}-${index}`}>
                <span className="tl-smart-record-icon" aria-hidden="true">{record.icon}</span>
                <span className="tl-smart-record-main">
                  <b>{record.subject} · {record.item}</b>
                  <small>{record.meta}</small>
                </span>
                <span className="tl-smart-record-value">{record.value}</span>
                {record.insight && <span className="tl-smart-record-insight">✦ {record.insight}</span>}
              </div>
            ))}
          </div>
          <section className="tl-smart-feedback" aria-label="整段输入的综合反馈">
            <div className="tl-smart-feedback-head"><b>关于这条点滴</b><span>综合 {records.length} 条记录</span></div>
            <p>{item.feedback}</p>
          </section>
        </div>
      )}
...
function TimelineItem({item, sisterItem, isNew, phaseKind, isFeedLast, sisterPlayAnimation, onSisterCycleComplete, firstDropAnim, onFirstDropLand, onFirstDropComplete}){
  ...
  } else if(item.kind === 'smart-capture'){
    const useStructuredDemo = new URLSearchParams(window.location.search).get('demo') === 'structured-timeline-v2';
    const StructuredCard = window.SmartCaptureStructuredCard;
    body = useStructuredDemo && StructuredCard
      ? <StructuredCard item={item} isNew={isNew}/>
      : <SmartCaptureTimelineCard item={item} isNew={isNew}/>;
  } else if(item.kind === 'focused-input-flow'){
    const FocusedInputFlowCard = window.FocusedInputFlowCard;
    body = FocusedInputFlowCard ? <FocusedInputFlowCard item={item} isNew={isNew}/> : null;
...
Object.assign(window, {
  ...
  MoodInsightCard, MoodCurveChart, MoodBandChart, BabyFeedingTimelineCard, SmartCaptureTimelineCard,
});

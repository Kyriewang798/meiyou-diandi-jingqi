...
function V3v2Card({primary, ai, aiDefaultOpen = false, isNew, staggerReveal = false, focusedStructuredFlow = false, entryId}){
  ...
  const scrollAfterAiChartReveal = React.useCallback(() => {
    if(focusedStructuredFlow) return;
    const run = () => {
  ...
  React.useEffect(() => {
    ...
    setRevealStep(0);
    setOpen(false);
    const tTags = setTimeout(() => setRevealStep(1), focusedStructuredFlow ? 280 : 380);
    const tAi = setTimeout(() => {
      setRevealStep(2);
      setOpen(true);
      setTimeout(scrollAfterAiChartReveal, 400);
    }, focusedStructuredFlow ? 1900 : 980);
    return () => {
      clearTimeout(tTags);
      clearTimeout(tAi);
    };
  }, [staggerReveal, isNew, aiDefaultOpen, primary?.id, focusedStructuredFlow, scrollAfterAiChartReveal]);
  ...
  return (
    ...
    >
      ...
      {focusedStructuredFlow && a && revealStep === 1 ? (
        <div className="tl-focus-processing tl-focus-structured-processing" role="status" aria-live="polite">
...
function V3RecordGroupCard({group, isNew}){
  ...
  return (
    <V3v2Card
      ...
      focusedStructuredFlow={!!group.focusedStructuredFlow}

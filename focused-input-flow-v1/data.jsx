...
const TIMELINE_BLOCKS = [
  ...
  },
  {
    ...
    items:[
      ...(new URLSearchParams(window.location.search).get('demo') === 'structured-timeline-v2' ? [{
        kind:'smart-capture', id:'e-today-smart-capture-bowel', time:'10:53',
        sourceKind:'text',
        sourceLabel:'来自文字记录',
        sourceText:'排便困难',
        feedbackDefaultOpen:true,
        records:[
          {
            icon:'💩', subject:'我', item:'便便', value:'排便困难', meta:'',
          },
        ],
      }] : []),
      ...(new URLSearchParams(window.location.search).get('demo') === 'structured-timeline-v2' ? [{
        kind:'smart-capture', id:'e-today-smart-capture-free-note', time:'18:30',
        sourceKind:'voice',
        duration:'0:04',
        sourceText:'我今天和小林出去玩。',
        records:[],
      }] : []),
      {
        kind:'smart-capture', id:'e-today-smart-capture', time:'19:00',
        duration:'0:12',
        sourceKind:'voice',
        sourceLabel:'来自 12 秒原声',
        sourceText:'宝宝今天的体重是9.3公斤，大女儿今天第二次来月经。血量挺多的，我今天挺担心的。',
        demoFlow:new URLSearchParams(window.location.search).get('demo') === 'structured-timeline-v2',
        defaultOpen:true,
        feedbackDefaultOpen:true,
        records:[
          {
            icon:'⚖', subject:'宝宝', item:'体重', value:'9.3 公斤', meta:'',
            analysis:{
              type:'weight-trend', title:'体重趋势', range:'近 4 次',
              values:[9.0, 9.1, 9.0, 9.3],
              labels:['8/20', '8/27', '9/3', '今天'],
              summary:'较上次 +0.3 公斤，整体呈上升趋势',
            },
          },
          { icon:'●', subject:'大女儿', item:'月经来了', value:'今天第 2 次', meta:'血量较多' },
          { icon:'♡', subject:'我', item:'心情', value:'担心', meta:'' },
        ],
        feedback:'宝宝体重保持上升；同时已记下大女儿本次经期血量较多，以及你的担心。后续有新记录时，会继续帮你串联变化。',
      },
    ],
  },
];
...
// 结构化时间流 Demo：历史记录也统一使用新信息组织规则。
function buildStructuredDemoTimeline(){
  const today = TIMELINE_BLOCKS.find(block=>block.type === 'day' && block.isToday);
  return [
    { type:'gap', id:'structured-gap-top' },
    {
      type:'day', id:'structured-d-9-8', date:'9/8', weekday:'周二',
      items:[
        {
          kind:'smart-capture', id:'structured-weight-0908', time:'08:20',
          sourceKind:'text', sourceLabel:'来自文字记录', sourceText:'今天体重 53.6 公斤。',
          records:[{icon:'⚖', subject:'我', item:'体重', value:'53.6 公斤', meta:''}],
        },
        {
          kind:'smart-capture', id:'structured-dinner-water-0908', time:'21:10',
          sourceKind:'voice', sourceLabel:'来自 8 秒原声', duration:'0:08',
          sourceText:'晚饭吃了沙拉，喝了两杯水。',
          records:[
            {icon:'🥗', subject:'我', item:'饮食', value:'晚饭·沙拉', meta:''},
            {icon:'💧', subject:'我', item:'喝水', value:'2 杯', meta:''},
          ],
        },
      ],
    },
    {
      type:'day', id:'structured-d-yesterday', date:'9/9', weekday:'周三', relativeLabel:'昨天',
      items:[
        {
          kind:'smart-capture', id:'structured-formula-yesterday', time:'07:47',
          sourceKind:'voice', sourceLabel:'来自 4 秒原声', duration:'0:04',
          sourceText:'宝宝早上喝了145毫升配方奶。',
          records:[{icon:'🍼', subject:'宝宝', item:'配方奶', value:'145 ml', meta:''}],
        },
        {
          kind:'smart-capture', id:'structured-diaper-yesterday', time:'15:10',
          sourceKind:'text', sourceLabel:'来自文字记录', sourceText:'宝宝换了尿布，臭臭。',
          records:[{icon:'🧷', subject:'宝宝', item:'换尿布', value:'臭臭', meta:''}],
        },
        {
          kind:'smart-capture', id:'structured-workout-yesterday', time:'18:10',
          sourceKind:'text', sourceLabel:'来自文字记录', sourceText:'去健身房练了四十分钟，出了一身汗。',
          records:[{icon:'🏃', subject:'我', item:'运动', value:'健身 40 分钟', meta:''}],
        },
        {
          kind:'smart-capture', id:'structured-baby-sleep-yesterday', time:'19:32',
          sourceKind:'voice', sourceLabel:'来自 9 秒原声', duration:'0:09',
          sourceText:'宝宝傍晚闹觉哭了快二十分钟，抱着走了两圈才睡着。',
          records:[
            {icon:'😴', subject:'宝宝', item:'睡眠', value:'已入睡', meta:'闹觉约 20 分钟'},
            {icon:'☹', subject:'宝宝', item:'心情', value:'哭闹', meta:''},
          ],
        },
      ],
    },
    today ? JSON.parse(JSON.stringify(today)) : buildTodayDayBlock(SCENE_CONTEXT.period),
  ];
}

// 聚焦版 Demo：只演示「输入 → 即时确认 → 提取 → 异步反馈」。
// 最终信息仍沿用线上时间轴的原文卡、记录卡、反馈卡层级，不改造信息架构。
function buildFocusedInputFlowTimeline(){
  const focusedDemoMode = new URLSearchParams(window.location.search).get('demo');
  const blocks = JSON.parse(JSON.stringify(TIMELINE_BLOCKS));
  return blocks.map(block=>{
    if(block.type !== 'day') return block;
    if(!block.isToday) return block;
    const items = (block.items || block.entries || [])
      .filter(item=>item.id !== 'e-today-smart-capture');
    items.push({
      kind:'focused-input-flow',
      id:'focused-input-flow-current',
      flowVariant:focusedDemoMode === 'focused-input-flow-v2' ? 'three-paths' : 'realtime',
      time:'19:00',
      sourceKind:'voice',
      duration:'0:12',
      sourceText:'宝宝今天的体重是9.3公斤，大女儿昨天晚上21:00第二次来月经。血量挺多的，我今天挺担心的。',
      records:[
        {icon:'⚖', subject:'宝宝', item:'体重', value:'9.3 公斤'},
        {icon:'●', subject:'大女儿', item:'月经来了', value:'昨天 21:00', meta:'第 2 次 · 血量较多'},
        {icon:'♡', subject:'我', item:'心情', value:'担心'},
      ],
...
window.buildStructuredDemoTimeline = buildStructuredDemoTimeline;
window.buildFocusedInputFlowTimeline = buildFocusedInputFlowTimeline;

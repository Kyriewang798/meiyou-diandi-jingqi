...
const STRUCTURED_DEMO_VOICE_LINE = '宝宝今天的体重是9.3公斤，大女儿今天第二次来月经。血量挺多的，我今天挺担心的。';
const FOCUSED_DEMO_VOICE_LINE = '宝宝今天的体重是9.3公斤，大女儿昨天晚上21:00第二次来月经。血量挺多的，我今天挺担心的。';
const FOCUSED_DEMO_TEXT_LINE = '我今天下午3点开始头痛，到现在还有一点恶心。';
...
function DockPublisher({
  ...
  structuredVoiceDemo = false, focusedInputDemo = false, focusedLegacyVoiceDemo = false,
  onPeriodStartQuick,
}){
  const I = window.Icon;
  ...
  const [focusedRecognizing, setFocusedRecognizing] = React.useState(false);
  ...
  const structuredRecordingRef = React.useRef(false);
  const focusedRecordingRef = React.useRef(false);
  const focusedRecognizeTimerRef = React.useRef(null);
  ...
  React.useEffect(()=>()=>clearTimeout(focusedRecognizeTimerRef.current), []);

  const startRec = ()=>{
    setRecording(true);
    if(focusedInputDemo){
      focusedRecordingRef.current = true;
      if(!focusedLegacyVoiceDemo){
        window.dispatchEvent(new CustomEvent('focusedDemoVoiceStart', {
          detail:{text:FOCUSED_DEMO_VOICE_LINE},
        }));
      }
      return;
    }
    if(structuredVoiceDemo){
      structuredRecordingRef.current = true;
      window.dispatchEvent(new CustomEvent('structuredDemoVoiceStart', {
        detail:{text:STRUCTURED_DEMO_VOICE_LINE},
      }));
      return;
    }
    onVoiceStart?.();
  ...
  const stopRec = ()=>{
    if(focusedInputDemo){
      if(!focusedRecordingRef.current) return;
      focusedRecordingRef.current = false;
      const recordedDuration = Math.max(recSec, 12);
      setRecording(false);
      setFocusedRecognizing(true);
      clearTimeout(focusedRecognizeTimerRef.current);
      focusedRecognizeTimerRef.current = setTimeout(()=>{
        setFocusedRecognizing(false);
        window.dispatchEvent(new CustomEvent('focusedDemoSubmit', {
          detail:{text:FOCUSED_DEMO_VOICE_LINE, duration:recordedDuration, sourceKind:'voice'},
        }));
      }, 720);
      return;
    }
    if(structuredVoiceDemo){
      if(!structuredRecordingRef.current) return;
      structuredRecordingRef.current = false;
      setRecording(false);
      window.dispatchEvent(new CustomEvent('structuredDemoVoiceEnd', {
        detail:{text:STRUCTURED_DEMO_VOICE_LINE, duration:Math.max(recSec, 3)},
      }));
      return;
    }
    if(!recording) return;
  ...
  const submitDockText = ()=>{
    const text = (draft || '').trim();
    if(!text) return;
    if(focusedInputDemo){
      onDraft('');
      window.dispatchEvent(new CustomEvent('focusedDemoSubmit', {
        detail:{text, sourceKind:'text'},
      }));
      return;
    }
    onSend();
  ...
  const toggleMode = ()=>{
    const nextMode = inputMode === 'text' ? 'voice' : 'text';
    if(nextMode === 'text' && focusedLegacyVoiceDemo && !(draft || '').trim()){
      onDraft(FOCUSED_DEMO_TEXT_LINE);
    }
  ...
  const handleDockQuickItemSelect = (item, buttonEl)=>{
    if(item?.action === 'period-start'){
      onPeriodStartQuick?.(item?.quickType);
      return;
    }
            ...
            <div className="dock-input-row dock-input-pill">
              ...
              {inputMode==='text' ? (
                ...
                <div className={'dock-voice-wrap'+(recording?' is-recording':'')}>
                  {/* 演示浮层指示器 */}
                  {(recording || (!focusedInputDemo && (focusedRecognizing || demoPhase === 'recognizing'))) && (
                    <div className={'dock-voice-float'+((!focusedInputDemo && (focusedRecognizing || demoPhase === 'recognizing')) ? ' is-recognizing' : '')}>
                      <span className="dock-voice-float-text">
                        {(!focusedInputDemo && (focusedRecognizing || demoPhase === 'recognizing')) ? '识别中...' : '正在听...'}
                      </span>
                      {!focusedInputDemo && (focusedRecognizing || demoPhase === 'recognizing') && (
                  ...
                  <button
                    ...
                    onPointerDown={(e)=>{ e.preventDefault(); if(isDemoRunning || focusedRecognizing) return; dismissGuideForVoice(); startRec(); }}
                    onPointerUp={stopRec}
              ...
              {inputMode==='text' && draft.trim() ? (
                <button type="button" className="dock-send-btn" onClick={submitDockText} aria-label="发送">

...
/** 卡片右上角 ··· 更多菜单 */
function CardMoreMenu({delayMs = 0, entryId, entryKind, onEdit, onDelete, editPayload}){
  ...
  return (
    <div className={'card-more-wrap'+(visible ? ' is-visible' : '')+(open ? ' is-open' : '')} ref={wrapRef}>
      <button
        type="button"
      ...
      {open && (
        <div className="card-more-menu">
          ...
          <button type="button" className="card-more-menu-item card-more-menu-item--danger" onClick={()=>{ setOpen(false); onDelete?.(entryId, entryKind); }}>
...
function PeriodFollowUpRow({entry, isNew, variant}){
  ...
  // 追问栏排在反馈正文之外，流式输出的自动滚动只盯正文，
  // 不管这一栏 —— 它会正好长在输入框底下。首次出现时自己滚进可视区。
  React.useLayoutEffect(()=>{
    if(!streamTitle || !rootRef.current) return;
  ...
  const titleNode = threadTitle
    ? threadTitle
    : (streamTitle
        ? <TypewriterText text={followUp.title} active charMs={90} followScroll/>
        : followUp.title);
...
function SisterAnalysisCollapsible({playAnimation, onCycleComplete, animateText, periodStyle = false, analysisKind = 'period-start', showPeriodFeelPrompt = true, periodFeelGuideCopy = PERIOD_FEEL_GUIDE_COPY, followUpEntry, followUpHostId, followUpIsNew, collapsed = false}){
  ...
  const headerTitle = isForecast ? FORECAST_TITLE : '本次月经分析';
    ...
    <>
      ...
      <section className="tl-t5-insight tl-sister-ai-wrap" id="sister-analysis-anchor">
        ...
        {/* 反馈内容下方的追问 / 查看按钮：流式输出结束才出现；
            反馈收起时一并收进去，不单独留在外面 */}
        {followUpEntry && hasSeenAnimation ? (
          <PeriodFollowUpRow
...
Object.assign(window, {
  ...
  SegmentedRecordCard, VoiceRecordCard, DemoVoiceCard, SisterAnalysisCard, SisterAnalysisCollapsible, SisterAnalysisContent, SisterForecastContent, scrollFeedContentIntoView,

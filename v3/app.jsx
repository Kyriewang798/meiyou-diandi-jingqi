...
function App(){
  ...
  const resetSceneState = (demoSceneId)=>{
    ...
    setEmptyPreviewGuideStep(0);
    setScene1Chat(null);
    clearTimeout(scene1RecordTimerRef.current);
    ...
    const syncEntry = {
      ...
      followUpEntry: (withFollowUp && !isPeriodEndAnalysis)
        ? {
            ...
            // 与时间轴第一轮一致：方案1 不带入上一轮，方案2 / 方案3 带
            contextText: scene.splitFeedback ? '今天月经来了' : undefined,
            contextKey: scene.splitFeedback ? 'analysis-feedback' : undefined,
  ...
  const submitVoice = (transcript, durSec)=>{
    ...
    if(scene.voiceFlow === 'ask' && window.createScene1AskEntry){
      ...
      const askOptions = {
        splitFeedback: !!scene.splitFeedback,
        feedbackInCard: !!scene.feedbackInCard,
        collapseOnLeave: !!scene.collapseOnLeave,
      };
      const makeFirstRound = scene.recordSummary && window.createScene1Plan3AskEntry
        ? window.createScene1Plan3AskEntry
      ...
      scrollTimelineToLastItem('smooth');
      setTimeout(()=>scrollTimelineToLastItem('smooth'), window.SCENE1_EXTRACT_MS + 120);
      return;
    }
  ...
  // v2 场景1：对话二级页（由时间轴卡片的「经期规律分析」入口打开）
  const [scene1Chat, setScene1Chat] = React.useState(null);
  // 对话页追问后延迟落轴的经期记录计时器：放在 App 层，返回点滴页不会取消，切换场景时清掉
  const scene1RecordTimerRef = React.useRef(null);
  ...
  useEffect(()=>{
    ...
    return ()=>window.removeEventListener('openScene1Chat', handleOpenScene1Chat);
  }, []);
  const showBottomTabBar = !homeDetailOpen && !scene1Chat;
  ...
  return (
    <>
      ...
      {scene1Chat && window.Scene1ChatPage ? (
        <window.Scene1ChatPage
          question={scene1Chat.question}
          title={scene1Chat.title}

...
function SleepExperienceComposer({onClose,onSave}){
  ...
  const steps={
    feeling:{prompt:'睡得怎么样？',options:['睡好了','没睡好','夜里老醒'],next:'detail'},
    detail:{prompt:'感受怎么样？',options:['身体乏力','神清气爽','提不起精神'],next:'context'},
    context:{prompt:'最近工作和生活发生了什么变化？',options:['压力大','加班熬夜','宠物打扰'],next:null}
  };
  ...
  const choose=f=>{setText(v=>v.trim()?(v===when?v+f:v+'，'+f):f);setStage(steps[stage].next);focus();};
  ...
  const save=()=>{
    ...
    onSave({datetime,time:datetime.slice(11),text:raw,sleepRecord:{sleep_event:raw,...(/午睡|小睡/.test(raw)?{sleep_type:'nap'}:/昨晚|夜里|晚上/.test(raw)?{sleep_type:'night'}:{}),scope:/最近|近期|这几天/.test(raw)?'recent':'unspecified'},scheme:'sleep-1'});
  };
    ...
    <div className="love-bottom">
      ...
      {stage&&<div className="love-time-bubbles love-options love-method-scroll">
        {stage==='time'?['昨晚','今天午睡'].map(t=><button key={t} onClick={()=>pickTime(t)}>{t}</button>):<>
          {steps[stage].options.map(f=><button key={f} onClick={()=>choose(f)}>{f}</button>)}
